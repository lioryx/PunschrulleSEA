...
-- 判断玩家身上是否包含列表中的任意装备
function HasItemInList()
	for slot = 1, 19 do
		local link = GetInventoryItemLink("player", slot)
		if link then
			local itemID = tonumber(string.match(link, "item:(%d+):"))
			if itemID and checkList[itemID] then
				return true
			end
		end
	end
...
function Punsch_Castbar_Update(e)
	...
	for i = 1, e.TicksShown do
		e.TickIndicators[i].texture:SetPoint("LEFT", e.self, "RIGHT", (-i * (e.self:GetWidth() / (e.TicksShown))) -
			(e.TickWidth / 2), 0)
		e.TickIndicators[i].texture:SetWidth(e.TickWidth)
	end
end
...
function Punsch_Castbar_OnEvent()
	...
	elseif (event == "SPELLS_CHANGED") then
		...
		DEFAULT_CHAT_FRAME:AddMessage("unhandled: " ..
			event .. ", " .. tostring(arg1) .. tostring(arg2) .. tostring(arg3) .. tostring(arg4))
	end
end
...
function Punsch_Castbar_HookUseAction(slot, checkCursor, onSelf)
	...
	if IsCurrentAction(slot) then
		if GetActionText(slot) == nil then
			...
			elseif (spellName == "稳固射击") then
				Punsch_Castbar_CastTrueShot()
			elseif (spellName == "精准射击") then
				Punsch_Castbar_CastTrueShot()
			elseif (spellName == "zzOLDTrueshot") then
				Punsch_Castbar_CastTrueShot()
			end
		end
		--lag, icons, rank
...
function Punsch_Castbar_CastTrueShot()
	local e = PunschEntities["Castbar"]
	if not e.isCasting and e.trueshot then
		Punsch_Castbar_OnCastStart("稳固射击", 1000)
	end
end
...
function Punsch_Castbar_OnUpdate()
	...
	if (e.isChannel and e.CountUpOnChannel) or (not e.isChannel and e.CountUpOnCast) then
		e.text2:SetText(string.format("%." .. e.decimals .. "f", e.duration - t) ..
			e.DurationTextSpacing .. "/" .. e.DurationTextSpacing .. string.format("%." .. e.decimals .. "f", e.duration))
	else
		e.text2:SetText(string.format("%." .. e.decimals .. "f", t) ..
			e.DurationTextSpacing .. "/" .. e.DurationTextSpacing .. string.format("%." .. e.decimals .. "f", e.duration))
	end
end
...
function Punsch_Castbar_OnChannelStart(name, duration)
	...
	if e.spellName ~= "正在引导" and e.TickEnable then
		...
		if tickinfo then
			if tickinfo.Tick == "time" then
				...
				if HasItemInList() then
					...
					tickinfo.TickCount = tickinfo.TickCount + 1
				...
				for i = 1, tickinfo.TickCount do
					if i == tickinfo.TickCount then
						...
						e.TickIndicators[i].texture:SetPoint("LEFT", e.self, "RIGHT",
							(-i * (e.self:GetWidth() / (tickinfo.TickCount))) - (e.TickWidth / 2), 0)
						e.TickIndicators[i].texture:SetWidth(e.TickWidth)
					end
...
function Punsch_Castbar_OnCastFailed()
	...
	if e.isChannel ~= true then
		e.isCasting = false
		if (e.spellName == "瞄准射击" or e.spellName == "多重射击" or e.spellName == "多重射击") and e.startTime - GetTime() < 0.08 then
...
function Punsch_Castbar_OnCastStart(name, duration)
	...
	elseif e.spellName == "多重射击" then
		...
		e.lag = nil
	elseif e.spellName == "稳固射击" then
		e.icontexture = "Interface\\Icons\\INV_Spear_06"
		e.lag = nil
	else
		e.lag, e.icontexture, e.rank = Punsch_Castbar_GetLastSpellInfo()
	end
