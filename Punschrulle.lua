	...
	if (event == "ADDON_LOADED") then
		if (arg1 == "Punschrulle") then
			...
			elseif not PunschrulleDB.Profiles[PunschrulleProfile] then
				PunschrulleProfile = "Default"
				DEFAULT_CHAT_FRAME:AddMessage("|cFF7373E6Punschrulle:|r 没有找到配置文件,使用配置文件 " ..
					PunschrulleDB.Profiles[PunschrulleProfile].Name)
			end

			if not PunschrulleDB.Profiles[PunschrulleProfile].MuteWelcomeMessage then
				DEFAULT_CHAT_FRAME:AddMessage("|cff00ffff读条计时 已加载|r /pch config");
			end
...
function Punschrulle_Command(msg)
	...
	elseif (cmd == "default") then
		...
		DEFAULT_CHAT_FRAME:AddMessage("|cFF7373E6Punschrulle:|r /pch config,default,lock");
	end
end
